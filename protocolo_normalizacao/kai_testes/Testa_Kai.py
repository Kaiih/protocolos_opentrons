...
def run(protocolo):
    ...
    try:
        ...
        protocolo.comment(f"CSV carregado com sucesso! {len(dados_biblioteca)} amostras encontradas.")
    ...
    for i, data in enumerate(dados_biblioteca):
        try:
            ...
            protocolo.comment(f"Erro nos dados da linha {i+1} do CSV: {e}. Pulando amostra {data.get('Sample Name', 'N/A')}.")
            continue
